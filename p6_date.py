...
time_t = Tuple[int, int]


def is_valid_datetime(input_string: str) -> bool:
    try:
        time.strptime(input_string, '%d/%m/%Y %H:%M')
        return True
    except ValueError:
        return False


def parse_datetime(input_string: str) -> Optional[Tuple[date_t, time_t]]:
    if not is_valid_datetime(input_string):
        return None

    date_, time_ = input_string.split(' ')
    date = tuple(map(int, date_.split('/')))
    day, month, year = date
    time = tuple(map(int, time_.split(':')))
    hour, minutes = time
